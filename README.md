...
hygge-match includes built-in evaluation so you can **measure matching performance and improve over time**. Ground truth is a DataFrame (or path to Parquet/CSV) with `left_id` and `right_id` columns listing known true pairs.
...
3. **Evaluate** — `metrics = results.evaluate(ground_truth)`. For deduplication use `right_id_col="id_right"`.
